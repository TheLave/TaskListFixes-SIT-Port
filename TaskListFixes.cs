...
namespace DrakiaXYZ.TaskListFixes
{
	...
	class TasksScreenShowQuestsPatch : ModulePatch
	{
		...
		private static Dictionary<Quest0, float> _questProgress = new Dictionary<Quest0, float>();
		...
		public static bool PatchPrefix(
			TasksScreen __instance, QuestControllerClass questController, ISession session, Func<Quest0, bool> ____questsAdditionalFilter,
			string ____currentLocationId, Profile ____activeProfile, Dictionary<Quest0, bool> ____questsAvailability, GameObject ____noActiveTasksObject,
			...
			object ____inventoryController)
		{
			...
			MethodInfo uiAddViewListMethod = AccessTools.Method(uiFieldInfo.FieldType, "AddViewList").MakeGenericMethod(new Type[] { typeof(Quest0), typeof(NotesTask) });
			...
			foreach (Quest0 questClass in questList)
			{
			...
			questList = questList.OrderBy(quest => quest, Comparer<Quest0>.Create((Quest0 questX, Quest0 questY) => ____questsAvailability[questY].CompareTo(____questsAvailability[questX])));
			...
			Action<Quest0, NotesTask> delegateInstance = (Quest0 quest, NotesTask view) =>
			{
		...
		private static bool FilterInGame(TasksScreen taskScreen, QuestControllerClass questController, Quest0 quest)
		{
			return (bool)_filterInGameMethod.Invoke(taskScreen, new object[] { questController, quest });
		}

		private class QuestLocationComparer : IComparer<Quest0>
		{
			...
			public int Compare(Quest0 quest1, Quest0 quest2)
			{
		...
		private class QuestTraderComparer : IComparer<Quest0>
		{
			public int Compare(Quest0 quest1, Quest0 quest2)
			{
				if (HandleNullOrEqualQuestCompare(quest1, quest2, out int result))
		...
		private class QuestTypeComparer : IComparer<Quest0>
		{
			public int Compare(Quest0 quest1, Quest0 quest2)
			{
				if (HandleNullOrEqualQuestCompare(quest1, quest2, out int result))
		...
		private class QuestTaskNameComparer : IComparer<Quest0>
		{
			public int Compare(Quest0 quest1, Quest0 quest2)
			{
				if (HandleNullOrEqualQuestCompare(quest1, quest2, out int result))
		...
		private class QuestStatusComparer : IComparer<Quest0>
		{
			public int Compare(Quest0 quest1, Quest0 quest2)
			{
				if (HandleNullOrEqualQuestCompare(quest1, quest2, out int result))
		...
		private class QuestProgressComparer : IComparer<Quest0>
		{
			public int Compare(Quest0 quest1, Quest0 quest2)
			{
				if (HandleNullOrEqualQuestCompare(quest1, quest2, out int result))
		...
		private static bool HandleNullOrEqualQuestCompare(Quest0 quest1, Quest0 quest2, out int result)
		{
			if (quest1 == quest2)
			{
				result = 0;
		...
		public static void SetQuestProgress(Quest0 quest, int progress)
		{
	...
	class QuestProgressViewPatch : ModulePatch
	{
		...
		[PatchPostfix]
		public static void PatchPostfix(Quest0 quest, TextMeshProUGUI ____percentages)
		{
